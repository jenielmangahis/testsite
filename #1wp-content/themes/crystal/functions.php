...
function graphene_setup() {
	...
	define( 'HEADER_IMAGE', apply_filters( 'graphene_header_image', '%s/images/headers/fireworks.jpg' ) );
	...
	function graphene_get_default_headers() {
		return array( 'Schematic' => array( 'url' => '%s/images/headers/schematic.jpg',
			...
			'Flow' => array( 'url' => '%s/images/headers/flow.jpg',
				...
				/* translators: header image description */
				'description' => __( 'This is the default Crystal theme header image, cropped from image by Quantin Houyoux at sxc.hu', 'graphene' ) ),
...
$style = '';
	...
	if ( $graphene_settings['menuround_corners_topleft'] != $graphene_defaults['menuround_corners_topleft'] ||
	     $graphene_settings['menuround_corners_topright'] != $graphene_defaults['menuround_corners_topright'] ||
		 $graphene_settings['menuround_corners_bottomleft'] != $graphene_defaults['menuround_corners_bottomleft'] ||
		 $graphene_settings['menuround_corners_bottomright'] != $graphene_defaults['menuround_corners_bottomright']){
		
		 $style .= '.menu{ border-top-left-radius:'.$graphene_settings['menuround_corners_topleft'].'px;border-top-right-radius:'.$graphene_settings['menuround_corners_topright'].'px;border-bottom-left-radius:'.$graphene_settings['menuround_corners_bottomleft'].'px;border-bottom-right-radius:'.$graphene_settings['menuround_corners_bottomright'].'px; }';
	}
	
	if ( $graphene_settings['page_border_color'] != $graphene_defaults['page_border_color'] ||
		 $graphene_settings['page_round_corners_topleft'] != $graphene_defaults['page_round_corners_topleft'] ||
	     $graphene_settings['page_round_corners_topright'] != $graphene_defaults['page_round_corners_topright'] ||
		 $graphene_settings['page_round_corners_bottomleft'] != $graphene_defaults['page_round_corners_bottomleft'] ||
		 $graphene_settings['page_round_corners_bottomright'] != $graphene_defaults['page_round_corners_bottomright'])
	$style .= '#container{border:solid '.$graphene_settings['page_border_size'].'px '.$graphene_settings['page_border_color'].' !important; 
	border-top-left-radius:'.$graphene_settings['page_round_corners_topleft'].'px;border-top-right-radius:'.$graphene_settings['page_round_corners_topright'].'px;border-bottom-left-radius:'.$graphene_settings['page_round_corners_bottomleft'].'px;border-bottom-right-radius:'.$graphene_settings['page_round_corners_bottomright'].'px;}';
	else
	$style .= '#container{border:solid '.$graphene_defaults['page_border_size'].'px '.$graphene_defaults['page_border_color'].' !important; }';
	
		if($graphene_settings['column_mode'] == 'one-column'):
			$style .= '#container {width:'.$graphene_settings['container_size'].'px !important; }';
			$style .= '.post {width:'.($graphene_settings['container_size']-85).'px !important; }';
			$style .= '.featured_slider #slider_root {width:'.($graphene_settings['container_size']-40).'px !important; }';
			$style .= '.slider_post {width:'.($graphene_settings['container_size']-70).'px !important; }';
		endif;
		if($graphene_settings['column_mode'] == 'two-col-left'):
			$style .= '#container {width:'.$graphene_settings['container_size'].'px !important; }';
			if($graphene_settings['content_size'] != $graphene_defaults['content_size']){
				$style .= '.two-columns #content-main {width:'.$graphene_settings['content_size'].'px !important; }';
				$style .= '.two-columns .featured_slider #slider_root {width:'.($graphene_settings['content_size']-45).'px !important; }';
				$style .= '.two-columns .slider_post{width:'.($graphene_settings['content_size']-85).'px !important; }';
				$style .= '.two-columns .post {width:'.($graphene_settings['content_size']-85).'px !important; }';
			}
			if($graphene_settings['leftsidebar_size'] != $graphene_defaults['leftsidebar_size'])
				$style .= '#sidebar1, #sidebar2 {width:'.$graphene_settings['leftsidebar_size'].'px !important; }';
		endif;
		if($graphene_settings['column_mode'] == 'two-col-right'):
			$style .= '#container {width:'.$graphene_settings['container_size'].'px !important; }';
			if($graphene_settings['content_size'] != $graphene_defaults['content_size']){
				$style .= '.two-columns #content-main {width:'.$graphene_settings['content_size'].'px !important; }';
				$style .= '.two-columns .featured_slider #slider_root {width:'.($graphene_settings['content_size']-45).'px !important; }';
			...
			if($graphene_settings['rightsidebar_size'] != $graphene_defaults['rightsidebar_size'])
				$style .= '#sidebar1, #sidebar2 {width:'.$graphene_settings['rightsidebar_size'].'px !important; }';
		endif;
		if($graphene_settings['column_mode'] == 'three-col-left' || $graphene_settings['column_mode'] == 'three-col-right' || $graphene_settings['column_mode'] == 'three-col-center'):
			$style .= '#container {width:'.$graphene_settings['container_size'].'px !important; }';
			if($graphene_settings['content_size'] != $graphene_defaults['content_size']){
				$style .= '.three-columns #content-main {width:'.$graphene_settings['content_size'].'px !important; }';
				$style .= '.three-columns .featured_slider #slider_root {width:'.($graphene_settings['content_size']-45).'px !important; }';
				$style .= '.three-columns .slider_post{width:'.($graphene_settings['content_size']-85).'px !important; }';
				$style .= '.three-columns .post {width:'.($graphene_settings['content_size']-85).'px !important; }';
			}
			if($graphene_settings['leftsidebar_size'] != $graphene_defaults['leftsidebar_size']){
				$style .= '#sidebar1 {width:'.$graphene_settings['leftsidebar_size'].'px !important; }';
				$style .= '.three-columns #sidebar1 div.sidebar-wrap {width:'.($graphene_settings['leftsidebar_size']-55).'px !important; }';
			}
			
			if($graphene_settings['rightsidebar_size'] != $graphene_defaults['rightsidebar_size'])
				$style .= '#sidebar2 {width:'.$graphene_settings['rightsidebar_size'].'px !important; }';
				$style .= '.three-columns #sidebar2 div.sidebar-wrap {width:'.($graphene_settings['rightsidebar_size']-55).'px !important; }';
		endif;
...
/*if(!function_exists('elcadia_exclude_categories')):
	function  elcadia_exclude_categories(){ global $graphene_settings; 
		 if(!empty($graphene_settings['sidebar_exclude_categories'])){ 
		 	$cat_args['exclude'] = $graphene_settings['sidebar_exclude_categories'];
		 }else{
		 	$cat_args['exclude'] = '';
		 }
	...
	add_filter('widget_categories_args', 'elcadia_exclude_categories', 10, 2);
endif;*/
...
if (!function_exists( 'graphene_default_menu' ) ) :
	...
	function graphene_default_menu(){ global $graphene_settings; ?>
		...
		<ul id="header-menu" class="menu clearfix default-menu">
            ...
            <?php endif; ?>
            <?php 
			    if(empty($graphene_settings['menuexclude_pages'])){ 
					$args = array( 
								'echo' => 1,
								'depth' => 5,
								'title_li' => '',
					...
					$args = array( 
								...
								'depth' => 5,
								'title_li' => '',
								'exclude' =>$graphene_settings['menuexclude_pages'],
								'walker' => new Walker_PageDescription() );
				}
...
/**
 * Gets all action hooks available in the Crystal theme.
...
/**
 * Display a dynamic widget area, this is hooked to the user selected do_action() hooks available in Crystal.
...
class Graphene_Widget_Twitter extends WP_Widget{
	...
	function Graphene_Widget_Twitter(){
		...
		// Create the widget
		$this->WP_Widget( 'graphene-twitter', 'Crystal Twitter', $widget_ops, $control_ops);
...
function graphene_options_init() {
	$graphene_options = add_theme_page(__( 'Crystal Options', 'graphene' ), __( 'Crystal Options', 'graphene' ), 'edit_theme_options', 'graphene_options', 'graphene_options' );
	$graphene_faq = add_theme_page(__( 'Crystal FAQs', 'graphene' ), __( 'Crystal FAQs', 'graphene' ), 'edit_theme_options', 'graphene_faq', 'graphene_faq' );
 ...
 * 
 * In Crystal version 1.3 the $noneText param has been removed
 *
 * @package WordPress
 * @subpackage Crystal
 * @since Crystal 1.3
...
function graphene_top_bar_social(){
    ...
    <?php endif;
	if ( $graphene_settings['youtube_url']) : ?>
    	<a href="<?php echo $graphene_settings['youtube_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Youtube channel", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="youtube_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Youtube channel", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['linkedin_url']) : ?>
    	<a href="<?php echo $graphene_settings['linkedin_url']; ?>" title="<?php printf(esc_attr__("Visit %s's LinkedIn page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="linkedin_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's LinkedIn page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['yahoo_url']) : ?>
    	<a href="<?php echo $graphene_settings['yahoo_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Yahoo page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="yahoo_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Yahoo page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['flickr_url']) : ?>
    	<a href="<?php echo $graphene_settings['flickr_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Flickr page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="flickr_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Flickr page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['vevo_url']) : ?>
    	<a href="<?php echo $graphene_settings['vevo_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Vevo page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="vevo_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Vevo page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['bing_url']) : ?>
    	<a href="<?php echo $graphene_settings['bing_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Bing page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="bing_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Bing page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
	if ( $graphene_settings['su_url']) : ?>
    	<a href="<?php echo $graphene_settings['su_url']; ?>" title="<?php printf(esc_attr__("Visit %s's Stumbleupon page", 'graphene' ), get_bloginfo( 'name' ) ); ?>" class="su_link" <?php if ( $graphene_settings['social_media_new_window'] ) { echo 'target="_blank"'; } ?>><span><?php printf(esc_attr__("Visit %s's Stumbleupon page", 'graphene' ), get_bloginfo( 'name' ) ); ?></span></a>
    <?php endif;
...
/**
	Widget for embed video from youtube, DailyMotion. etc.
*/


class VideoEmbedWidget extends WP_Widget
{
  function VideoEmbedWidget()
  {
    $widget_ops = array('classname' => 'VideoEmbedWidget', 'description' => 'Widget for embed video from Youtube, DailyMotion, etc.' );
    $this->WP_Widget('VideoEmbedWidget', 'Crystal Add Video', $widget_ops);
  }
 
  function form($instance)
  {
    $instance = wp_parse_args( (array) $instance, array( 'title' => '','website'=>'','width'=>'', 'height'=>'','link'=>'','embedcode'=>'') );
...
?>
  ...
  <p>
  <label for="<?php echo $this->get_field_id('link'); ?>">Video Link: 
  ...
  <p>
  If you select Blip.Tv please insert embed code.<br />
  <label for="<?php echo $this->get_field_id('embedcode'); ?>">Embed Code:<br />
  	<textarea name="<?php echo $this->get_field_name('embedcode'); ?>" id="<?php echo $this->get_field_id('embedcode'); ?>" cols="28" rows="4" style="resize:none"><?php echo attribute_escape($embedcode); ?></textarea>
  </label>
  </p>
  <p>
  <label for="<?php echo $this->get_field_id('website'); ?>">Website:
  	<select name="<?php echo $this->get_field_name('website'); ?>" id="<?php echo $this->get_field_id('website'); ?>">
    	<option value="youtube" <?php if(attribute_escape($website) == 'youtube') echo 'selected="selected"'; ?>>Youtube</option>
        <option value="dailymotion" <?php if(attribute_escape($website) == 'dailymotion') echo 'selected="selected"'; ?>>DailyMotion</option>
        <option value="vevo" <?php if(attribute_escape($website) == 'vevo') echo 'selected="selected"'; ?>>Vevo</option>
        <option value="yahoovideo" <?php if(attribute_escape($website) == 'yahoovideo') echo 'selected="selected"'; ?>>Yahoo Video</option>
        <option value="photobucket" <?php if(attribute_escape($website) == 'photobucket') echo 'selected="selected"'; ?>>PhotoBucket</option>
        <option value="metacafe" <?php if(attribute_escape($website) == 'metacafe') echo 'selected="selected"'; ?>>Metacafe</option>
        <option value="blip" <?php if(attribute_escape($website) == 'blip') echo 'selected="selected"'; ?>>Blip.Tv</option>
    </select> 
  </label>
  </p>
  <p>
...
<?php
  ...
  function update($new_instance, $old_instance)
  {
	...
	$instance['embedcode'] = $new_instance['embedcode'];
    return $instance;
  }
    ...
    if (!empty($title))
 	...
 	if(!empty($instance['link'])){
		...
		switch($instance['website']){
			case 'youtube':
				if(empty($instance['width'])) $instance['width'] = 200;
				if(empty($instance['height'])) $instance['height'] = 200;
				$variables = $this->getVariableFromUrl($instance['link']);
				echo '<iframe width="'.$instance['width'].'" height="'.$instance['height'].'" src="http://www.youtube.com/embed/'.$variables['v'].'" frameborder="0" allowfullscreen></iframe>';
			break;
			case 'dailymotion':
				$output = parse_url($instance['link']);
				$url= $output['path'];
				$parts = explode('/',$url);
				...
				echo '<iframe frameborder="0" width="'.$instance['width'].'" height="'.$instance['height'].'" src="http://www.dailymotion.com/embed/video/'.$parts[0].'"></iframe>';
			break;
			case 'vevo':
				$output = parse_url($instance['link']);
				$url= $output['path'];
				...
				echo '<object width="'.$instance['width'].'" height="'.$instance['height'].'">
				<param name="movie" value="http://www.vevo.com/VideoPlayer/Embedded?videoId='.$parts[4].'&playlist=false&autoplay=0&playerId=62FF0A5C-0D9E-4AC1-AF04-1D9E97EE3961&playerType=embedded&env=0"></param>
				<param name="wmode" value="transparent"></param>
				<param name="bgcolor" value="#000000"></param>
				<param name="allowFullScreen" value="true"></param>
				<param name="allowScriptAccess" value="always"></param>
				<embed src="http://www.vevo.com/VideoPlayer/Embedded?videoId='.$parts[4].'&playlist=false&autoplay=0&playerId=62FF0A5C-0D9E-4AC1-AF04-1D9E97EE3961&playerType=embedded&env=0" type="application/x-shockwave-flash" 
				allowfullscreen="true" allowscriptaccess="always" width="'.$instance['width'].'" height="'.$instance['height'].'" bgcolor="#000000" wmode="transparent"></embed>
				</object> ';
			break;
			case 'yahoovideo':
				$output = parse_url($instance['link']);	
				
				if(isset($output['path']) && !empty($output['path'])){		
					$parts = explode('/',$output['path']);
					
					if(isset($parts[3])){
						$video = substr(strrchr($parts[3], '-'),1);
					}else{
						$video = substr(strrchr($parts[1], '-'),1);
					}
					$video = explode('.',$video);
				...
				if(isset($variables['vid'])){
					echo '<iframe frameborder="0" width="'.$instance['width'].'" height="'.$instance['height'].'" src="http://d.yimg.com/nl/movies/site/player.html#vid='.$variables['vid'].'&shareUrl='.$instance['link'].'"></iframe>';
				}elseif(isset($videoID) && !empty($videoID)){
					echo '<iframe frameborder="0" width="'.$instance['width'].'" height="'.$instance['height'].'" src="http://d.yimg.com/nl/movies/site/player.html#vid='.$videoID.'&shareUrl='.$instance['link'].'"></iframe>';
				}else{
					echo 'Sorry but this video was not found.';
				}
			break;
			case 'photobucket':		
				if(empty($instance['width'])) $instance['width'] = 200;
				if(empty($instance['height'])) $instance['height'] = 200;	
				echo '<embed width="'.$instance['width'].'" height="'.$instance['height'].'" type="application/x-shockwave-flash" allowFullscreen="true" allowNetworking="all" wmode="transparent" 
					  src="http://static.photobucket.com/player.swf" flashvars="file='.$instance['link'].'">';
			break;
			case 'metacafe':
				if(empty($instance['width'])) $instance['width'] = 200;
				if(empty($instance['height'])) $instance['height'] = 200;
				...
				if(isset($parts[2]) && isset($parts[3])){
					echo '<embed flashVars="playerVars=autoPlay=no" src="http://www.metacafe.com/fplayer/'.$parts[2].'/'.$parts[3].'.swf" width="'.$instance['width'].'" height="'.$instance['height'].'" wmode="transparent" allowFullScreen="true" 
					allowScriptAccess="always" name="Metacafe_7655580" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"></embed>';
				}
			break;
		}
	}else{
		if($instance['website'] == 'blip'){
				if(!empty($instance['embedcode']))
					echo $instance['embedcode'];
		}
	}
...
add_action( 'widgets_init', create_function('', 'return register_widget("VideoEmbedWidget");') );


function optin_dashboard_widget_function() {
	// Display whatever it is you want to show
?>
<p style="text-align:center;">Subscribe to receive lifetime updates and upgrade of Crystal Theme.</p>
...
<div style="display: none;"><img src="http://forms.aweber.com/form/displays.htm?id=rAwsjMwcTIxM" alt="" /></div>
</form>
<script type="text/javascript">
<!--
     (function() {
...
function optin_add_dashboard_widgets() {
	wp_add_dashboard_widget('optin_dashboard_widget', 'Subscribe Form', 'optin_dashboard_widget_function');	
} 

// Hook into the 'wp_dashboard_setup' action to register our other functions

add_action('wp_dashboard_setup', 'optin_add_dashboard_widgets' );
...
require_once (TEMPLATEPATH . '/plugins/contactdetails-widget/contactdetails-widget.php');

/**
	Optin Form Widget
*/

require_once (TEMPLATEPATH . '/plugins/optin-form-widget/optin-form-widget.php');

/**
	Categories Widget
*/

require_once (TEMPLATEPATH . '/plugins/categories-widget/categories-widget.php');
